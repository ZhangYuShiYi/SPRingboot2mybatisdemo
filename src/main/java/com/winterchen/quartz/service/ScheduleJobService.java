package com.winterchen.quartz.service;

import com.winterchen.dao.UserDao;
import com.winterchen.quartz.dao.ScheduleJobDaoRepository;
import com.winterchen.quartz.entity.model.ScheduleJobModel;
import com.winterchen.quartz.entity.po.ScheduleJobPo;
import com.winterchen.quartz.job.TestQuartzJob;
import com.winterchen.quartz.util.DateUtil;
...
public class ScheduleJobService {
    ...
    public void startSchedule(ScheduleJobModel model) {
        ...
        try {
            ...
            Integer id = userDao.getMaxIdInScheduleJob();
            /*Long id = SnowFlake.getSnowFlake().nextId();*/
            scheduleJobPo.setId(id+1);
    ...
    private void startJob(Scheduler scheduler, String group, String name, String cron) throws SchedulerException {
        ...
        map.put("key2", "val2");*/
        // JobDetail 是具体Job实例
//        JobDetail jobDetail = JobBuilder.newJob(TestQuartzJob.class).withIdentity("h1","t1")
        JobDetail jobDetail = JobBuilder.newJob(TestQuartzJob.class).withIdentity(name, group)
                .usingJobData(map)
                .build();
        // 基于表达式构建触发器
        ...
        CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity(name, group)/*.withIdentity("testQuartz")*/
                .withSchedule(cronScheduleBuilder).build();
